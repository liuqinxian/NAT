...
class Bottleneck(nn.Module):
    def __init__(self, last_planes, in_planes, out_planes, dense_depth, stride, first_layer):
        ...
        self.conv2 = nn.Conv3d(in_planes, in_planes, kernel_size=3, stride=stride, padding=1, groups=4, bias=False)
...
class DPN(nn.Module):
    def __init__(self, cfg):
        ...
        # self.linear = nn.Linear(out_planes[0]+(num_blocks[0]+1)*dense_depth[0], 2)#10)
        self.avg = nn.AvgPool3d(kernel_size=3, stride=3)
        self.linear = nn.Linear(32768, 1)#10)
